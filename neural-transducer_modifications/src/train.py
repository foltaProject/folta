...
class Arch(util.NamedEnum):
    ...
    # hard attention with REINFORCE approximation without input-feeding
    approxihard = 'approxihard'
    softinputfeed = 'softinputfeed'  # soft attention with input-feeding
    # soft attention with uncontrolled input-feeding
    largesoftinputfeed = 'largesoftinputfeed'
    # hard attention with REINFORCE approximation with input-feeding
    approxihardinputfeed = 'approxihardinputfeed'
...
class Trainer(BaseTrainer):
    ...
    def load_data(self, dataset, train, dev, test):
        ...
        if params.arch == Arch.hardmono:
            if dataset == Data.sigmorphon17task1:
                self.data = dataloader.AlignSIGMORPHON2017Task1(
                    train, dev, test, params.shuffle)
            elif dataset == Data.g2p:
                self.data = dataloader.AlignStandardG2P(
                    train, dev, test, params.shuffle)
            ...
            if dataset == Data.sigmorphon17task1:
                if params.indtag:
                    self.data = dataloader.TagSIGMORPHON2017Task1(
                        train, dev, test, params.shuffle)
                else:
                    self.data = dataloader.SIGMORPHON2017Task1(
                        train, dev, test, params.shuffle)
            elif dataset == Data.unimorph:
                if params.indtag:
                    self.data = dataloader.TagUnimorph(
                        train, dev, test, params.shuffle)
                else:
                    self.data = dataloader.Unimorph(
                        train, dev, test, params.shuffle)
            elif dataset == Data.sigmorphon19task1:
                assert isinstance(train, list) and len(
                    train) == 2 and params.indtag
                self.data = dataloader.TagSIGMORPHON2019Task1(
                    train, dev, test, params.shuffle)
            elif dataset == Data.sigmorphon19task2:
                assert params.indtag
                self.data = dataloader.TagSIGMORPHON2019Task2(
                    train, dev, test, params.shuffle)
            elif dataset == Data.g2p:
                self.data = dataloader.StandardG2P(
                    train, dev, test, params.shuffle)
            elif dataset == Data.p2g:
                self.data = dataloader.StandardP2G(
                    train, dev, test, params.shuffle)
            elif dataset == Data.news15:
                self.data = dataloader.Transliteration(
                    train, dev, test, params.shuffle)
            elif dataset == Data.histnorm:
                self.data = dataloader.Histnorm(
                    train, dev, test, params.shuffle)
            elif dataset == Data.sigmorphon16task1:
                if params.indtag:
                    self.data = dataloader.TagSIGMORPHON2016Task1(
                        train, dev, test, params.shuffle)
                else:
                    self.data = dataloader.SIGMORPHON2016Task1(
                        train, dev, test, params.shuffle)
            elif dataset == Data.lemma:
                if params.indtag:
                    self.data = dataloader.TagLemmatization(
                        train, dev, test, params.shuffle)
                else:
                    self.data = dataloader.Lemmatization(
                        train, dev, test, params.shuffle)
            elif dataset == Data.lemmanotag:
                self.data = dataloader.LemmatizationNotag(
                    train, dev, test, params.shuffle)
            else:
                raise ValueError
        # yapf: enable
    ...
    def build_model(self):
        ...
        self.model = model_class(**kwargs)      # This is where the model is instantiated in transformer.py
        if params.indtag:
    ...
    def evaluate(self, mode, epoch_idx, decode_fn):
        ...
        for result in results:
            self.logger.info(
                ...
                # fix alexander kahanek
                '{0} {1} is {2} at epoch {3}'.format(
                    mode, result.long_desc, result.res, epoch_idx)
            )
        return results
